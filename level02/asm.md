```
   ...
   0x0000000000400818 <+4>:	sub    $0x120,%rsp - 288 bytes
   0x000000000040081f <+11>:	mov    %edi,-0x114(%rbp)
   0x0000000000400825 <+17>:	mov    %rsi,-0x120(%rbp)
```
<0> - <17>: in x86 calling convention, edi 1 arg (argc), rsi 2 arg(av**) and is macking a copy of them
```
   ...
   stos will write 8 bytes at a time => 96 zeros
   0x0000000000400840 <+44>:	mov    %rdi,%rdx
   0x0000000000400843 <+47>:	mov    %eax,(%rdx)
   0x0000000000400845 <+49>:	add    $0x4,%rdx
   + 4 zeros to probably fill in the rest of the buffer => buffer1 is of size 100
```
<24> - <49>: memset( buf1 , 0, 100);
```
   0x0000000000400849 <+53>:	lea    -0xa0(%rbp),%rdx
   0x0000000000400850 <+60>:	mov    $0x0,%eax
   0x0000000000400855 <+65>:	mov    $0x5,%ecx - 5b
...
```
<117> - <153> :file_v = fopen("/home/users/level03/.pass", "r");
```
...
if (file_v == 0 )
```
   0x00000000004008b8 <+164>:	mov    0x200991(%rip),%rax        # 0x601250 <stderr@@GLIBC_2.2.5> - stderr is set by fopen in case of failure
...
```
<164> - <205>: fwrite(1, "ERROR: failed to open password file\n", 36, 1);
              exit(1);
```
   0x00000000004008e6 <+210>:	lea    -0xa0(%rbp),%rax -buf2
...
```
<210> - <242>: ret = fread( buf2, 1,  41,  file);
```
   0x0000000000400909 <+245>:	lea    -0xa0(%rbp),%rax
   0x0000000000400910 <+252>:	mov    $0x400bf5,%esi - "/n"
...
prtinf("--[ Userneme:")
   ...
   0x00000000004009ca <+438>:	mov    %rax,%rdx - 0
   0x00000000004009cd <+441>:	lea    -0x70(%rbp),%rax -buf1
...
puts("*************************************");
   ...
   0x0000000000400a4a <+566>:	lea    -0x110(%rbp),%rcx -buf1
   0x0000000000400a51 <+573>:	lea    -0xa0(%rbp),%rax - buf2
