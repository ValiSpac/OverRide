...
```
   ...
   0x0804887f <+6>:	sub    $0x50,%esp - 80 bytes for loc var
   0x08048882 <+9>:	mov    0xc(%ebp),%eax
   0x08048885 <+12>:	mov    %eax,0x1c(%esp)
   0x08048889 <+16>:	mov    %gs:0x14,%eax
   0x0804888f <+22>:	mov    %eax,0x4c(%esp) - int i
```
   gs segment register that point to Thread Local Storage(TLS),
   works as a stack protector against buffer overflows
```
   0x08048893 <+26>:	xor    %eax,%eax - zero
...
```
<245> - <258>: if (canary != \*(unsigned int \*)(\_\_gs + 0x14))
\_\_stack_check_failed();
```
   0x08048980 <+263>:	leave  
   0x08048981 <+264>:	ret    
```

auth(int serial, char \*buf):
```
   0x08048748 <+0>:	push   %ebp
   0x08048749 <+1>:	mov    %esp,%ebp
   0x0804874b <+3>:	sub    $0x28,%esp - 40 bytes loc var
...
```
<6> - <20>: buf\[strcspn(buf, "\n")] = 0;
```
   0x08048761 <+25>:	add    0x8(%ebp),%eax
   0x08048764 <+28>:	movb   $0x0,(%eax) - *buf = 0
   0x08048767 <+31>:	movl   $0x20,0x4(%esp) - var1 = 32
...
```
<78> - <117>: if (ptrace(0, 0, 1, 0) == 0xffffffff) 
```
...
```
   ...
   0x08048806 <+190>:	movl   $0x0,-0x14(%ebp) - i =0
   ...
   0x08048818 <+208>:	cmp    $0x1f,%al - printable char else ret 1
   ...
   0x08048848 <+256>:	imul   $0x539,%eax,%eax
    x % 1337  ≡  x - (x * 0x88233b2b >> 32, adjusted) * 1337
    compiler optimisation for "% 1337"
   ...
   0x0804885b <+275>:	mov    -0x14(%ebp),%eax - beginign of loop
   0x0804885e <+278>:	cmp    -0xc(%ebp),%eax
   0x08048861 <+281>:	jl     0x804880f <auth+199> - i < len
```
<187 - <281>:
```
for (int i = 0; i < len, i++)
{
        if ((unsigned char)buf[i] <= 0x1f)#serial must be only printable chars
            return 1;
